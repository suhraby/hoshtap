            ...
            <form @submit.prevent="onSubmit">
                    ...
                    >
                        <div v-for="locale in locales" :key="locale.code">
                            ...
                            <InputField
                                ...
                                :placeholder="`${$t('Client title')} — ${locale.label}`"
                            />
                            <InputError
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        <InputLabel :value="$t('Client image')" />
                ...
                <div class="mt-6 flex items-center justify-end gap-3">
                    <Link
                        :href="route('manage.clients.index')"
...
import type { Client, ResourceResponse } from '@/types';
...
const props = defineProps<{
    client: ResourceResponse<Client>;
}>();

const currentPageTitle = computed(() =>
    trans('Edit client: :title', {
        title:
            props.client.data.title[lang.value] ??
            props.client.data.title['en'],
    }),
);
...
const form = useForm({
    title: Object.fromEntries(
        locales.value.map((l) => [
            l.code,
            props.client.data.title[l.code] ?? '',
        ]),
...
function onSubmit(): void {
    form.patch(route('manage.clients.update', props.client.data.id), {
        preserveScroll: true,
    });
}
</script>
