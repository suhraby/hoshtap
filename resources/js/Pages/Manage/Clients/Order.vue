...
const currentPageTitle = wTrans('Sort clients');

const sortedClients = ref<Client[]>([...props.clients.data]);
...
function onDragOver(_e: DragEvent, index: number): void {
    ...
    const items = [...sortedClients.value];
    const dragged = items.splice(draggingIndex.value, 1)[0];
    items.splice(index, 0, dragged);

    sortedClients.value = items;
    draggingIndex.value = index;
}
...
function onSubmit(): void {
    form.ids = sortedClients.value.map((b) => b.id);

    form.patch(route('manage.clients.sortOrder'), {
        preserveScroll: true,
    });
}
</script>
