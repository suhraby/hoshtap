        ...
        <ComponentCard>
            <form @submit.prevent="onSubmit">
                <ul class="space-y-3" ref="listRef">
                    <li
                        v-for="(counter, index) in sortedCounters"
                        :key="counter.id"
                ...
                <div class="mt-6 flex items-center justify-end gap-3">
                    <Link
                        :href="route('manage.counters.index')"
...
const currentPageTitle = wTrans('Sort counters');

const sortedCounters = ref<Counter[]>([...props.counters.data]);
...
function onDragOver(_e: DragEvent, index: number): void {
    ...
    const items = [...sortedCounters.value];
    const dragged = items.splice(draggingIndex.value, 1)[0];
    items.splice(index, 0, dragged);

    sortedCounters.value = items;
    draggingIndex.value = index;
}
...
function onSubmit(): void {
    form.ids = sortedCounters.value.map((b) => b.id);

    form.patch(route('manage.counters.sortOrder'), {
        preserveScroll: true,
    });
}
</script>
