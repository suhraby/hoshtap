...
const currentPageTitle = wTrans('Sort banners');

const sortedBanners = ref<Banner[]>([...props.banners.data]);
...
function onDragOver(_e: DragEvent, index: number): void {
    ...
    const items = [...sortedBanners.value];
    const dragged = items.splice(draggingIndex.value, 1)[0];
    items.splice(index, 0, dragged);

    sortedBanners.value = items;
    draggingIndex.value = index;
}
...
function onSubmit(): void {
    form.ids = sortedBanners.value.map((b) => b.id);

    form.patch(route('manage.banners.sortOrder'), {
        preserveScroll: true,
    });
}
</script>
